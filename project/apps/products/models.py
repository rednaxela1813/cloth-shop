...
class Product(models.Model):
    ...
    def _sorted_prefetched_images(self):
        ...
        """
        prefetched_images = getattr(self, "_prefetched_images_for_primary", None)
    ...
    def _active_variants_for_default_selection(self):
        ...
        if prefetched is None:
            prefetched = getattr(self, "_prefetched_active_variants_for_selection", None)
        if prefetched is None:
            prefetched = getattr(self, "_prefetched_active_variants_for_pricing", None)
        if prefetched is not None:
            return prefetched
...
class ProductVariant(models.Model):
    ...
    def _prefetched_images(self):
        ...
        if prefetched is not None:
            return sorted(prefetched, key=lambda image: (image.sort_order, image.id))
